...
1. [Deployment record](#deployment-record)
...
Deployed via `azd up` — Day 5 Task 4 (`day-5task-4` branch).

**Live endpoint:** `https://quotesapi-dev.whitestone-71ebd55e.centralindia.azurecontainerapps.io/`

![quotesapi-dev live endpoint](docs/quotesapi-dev-live-endpoint.png)

This shares the `thinkschool-env` Container Apps environment (in `thinkschool-rg`) via `AZURE_CONTAINER_APPS_ENVIRONMENT_ID`, since the subscription caps Container Apps environments at one per region and Central India's slot was already taken. The container app itself lives in its own `rg-quotesapi-dev` resource group (ACR, Log Analytics, Application Insights, managed identity).
