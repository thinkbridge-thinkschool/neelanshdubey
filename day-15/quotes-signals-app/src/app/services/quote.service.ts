 ...
 *
 * The bearer token, GET retries, and ProblemDetails-to-AppError mapping all
 * happen in interceptors/ (registered in app.config.ts) rather than here —
 * this service just describes the requests.
 */
...
export class QuoteService {
  ...
  createQuote(request: CreateQuoteRequest): Promise<Quote> {
    return firstValueFrom(this.http.post<Quote>('/api/quotes', request));
  }

  updateQuote(id: number, request: UpdateQuoteRequest): Promise<Quote> {
    return firstValueFrom(this.http.put<Quote>(`/api/quotes/${id}`, request));
  }

  deleteQuote(id: number): Promise<void> {
    return firstValueFrom(this.http.delete<void>(`/api/quotes/${id}`));
  }
}
